 ...
 * @brief Implements some base functions of the HD44780 family and its successors RW1073 and SSD1803(A)
 * @version 1.0
 * @date 2024-04-18
 *
...
HD44780::HD44780(int8_t my_ss, int8_t my_miso, int8_t my_mosi, int8_t my_sck, uint8_t my_reset_pin)
...
void HD44780::setPos(uint8_t x, uint8_t y) {
    // TODO: This seems to work on 5-dot font width.
    // Is this in contrast to a 6-dot font?
    switch (_rows) {
        case R4:
            y = (y < R4) ? y : y % R4;
            x = (x < (80 / R4)) ? x : x % (80 / R4);
...
RW1073::RW1073(int8_t my_ss, int8_t my_miso, int8_t my_mosi, int8_t my_sck, uint8_t my_reset_pin)
...
void RW1073::setBlinking(uint8_t loc, uint8_t blink, uint8_t pattern) {
    loc &= 0x0f;
    blink &= 0x03;
    pattern &= 0x1f; // Always 5-dot font. TODO: character width dependent.
...
void RW1073::iconBlink(bool en) {
    if (en)
        ...
        _instr1_function_set &= ~INSTR1_FS_BE;
    _pdto->ir_write(_instr1_function_set);
    _re_set = true;
    _pdto->ir_write(_instr0_function_set);
    _re_set = false;
}

void RW1073::testingScrolling() {
    _instr1_shift_enable |= INSTR1_SE_DS4;
    _instr0_entry_mode_set |= INSTR0_EMS_S;

    _pdto->ir_write(_instr1_function_set | INSTR1_FS_BE);
    _re_set = true;
    _pdto->ir_write(_instr1_shift_enable);
    _pdto->ir_write(_instr0_function_set);
    _re_set = false;
    _pdto->ir_write(_instr0_entry_mode_set);
    setXY(10, 3);
    const char MSG[40] = "Hallo Viete! Mal sehen, ob es klappt...";
    for (int i = 0; i < 39; i++) {
        write(MSG[i]);
        delay(250);
    }
...
SSD1803A::SSD1803A(int8_t my_ss, int8_t my_miso, int8_t my_mosi, int8_t my_sck,
                                           uint8_t my_reset_pin)
