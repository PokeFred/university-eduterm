...
public:
    HD44780(int8_t my_ss, int8_t my_miso, int8_t my_mosi, int8_t my_sck, uint8_t my_reset_pin);
    HD44780(const HD44780 &) = delete;
    void operator=(const HD44780 &) = delete;
    ~HD44780() override = default;
...
public:
    RW1073(int8_t my_ss, int8_t my_miso, int8_t my_mosi, int8_t my_sck, uint8_t my_reset_pin = 0);
    RW1073(const RW1073 &) = delete;
    void operator=(const RW1073 &) = delete;
    ~RW1073() override = default;

    virtual void begin(row_t rows, col_t cols);
    void setBlinking(uint8_t loc, uint8_t blink, uint8_t pattern);
    void iconBlink(bool en);
    void testingScrolling();
    void testingShowIcon();
    void powerDown(bool en);

protected:
    virtual void init(row_t rows, col_t cols);
    ...
    static constexpr uint8_t INSTR1_EMS_BID = 0x01; // bidirectional Seg60->1/Seg1->60
    ...
    static constexpr uint8_t INSTR1_SE_DS3 = 0x04; // enable 3rd line display shift
...
class __attribute__((unused)) SSD1803A : public RW1073 {
public:
    SSD1803A(int8_t my_ss, int8_t my_miso, int8_t my_mosi, int8_t my_sck,
             uint8_t my_reset_pin = 0);
    SSD1803A(const SSD1803A &) = delete;
    void operator=(const SSD1803A &) = delete;
    ~SSD1803A() override = default;

    void begin(row_t rows, col_t cols) override;

protected:
    void init(row_t rows, col_t cols) override;
