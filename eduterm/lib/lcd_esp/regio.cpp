...
SPIOps::SPIOps(int8_t my_ss, int8_t my_miso, int8_t my_mosi, int8_t my_sck)
...
bool SPIOps::ir_write(const void *comd, uint8_t size) const {
    ...
    for (uint_fast8_t i = 0; i < size; i++) {
        ...
        const auto *cmd = static_cast<const uint8_t *>(comd);
        spi_buf[0] = INSTR_WRT;
...
uint8_t SPIOps::bfa_read() const {
    ...
    const uint16_t TX_WORD = BUSY_RD;
    // Reading busy register from bus
    vspi->beginTransaction(SPISettings(VSPI_CLOCK, VSPI_BIT_ORDER, VSPI_DATA_MODE));
    digitalWrite(vspi->pinSS(), LOW);
    uint16_t rx = vspi->transfer16(TX_WORD);
    digitalWrite(vspi->pinSS(), HIGH);
    vspi->endTransaction();
    return rx >> 8;
}
...
bool SPIOps::dr_read(void *dst, uint8_t size) const {
    ...
    auto *data = static_cast<uint8_t *>(dst);
    vspi->beginTransaction(SPISettings(VSPI_CLOCK, VSPI_BIT_ORDER, VSPI_DATA_MODE));
    digitalWrite(vspi->pinSS(), LOW);
